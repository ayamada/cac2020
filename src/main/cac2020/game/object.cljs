...
;;; TODO
;;; - 以下の改善を行う前にversionに-ADVANCEDをつけておく事
;;; - 白cubeの射出位置が上すぎる、調整する事
;;; - 敵や弾に衝突した時のスマホ振動量を増やす事
;;; - もうちょっと:cherryを多く配置する？
...
(defobj :proceed-bullet
  ...
  :move-fn (fn [a-state ^js o delta-frames]
             ...
             (let [y (.-y o)]
               (when (or
                       ;; TODO: 弾丸に限り、xが右に行き過ぎても消したい。しかし判定が面倒(自機からの相対距離での判定が必要)
                       (< y -1024)
                       (< 512 y))
                 (putil/dea! o))))
  :collide-fn cube-collide!)
...
;;;         - bullet (主人公が射出するもの) - 敵に当たると倒して得点。見た目は小さい:bananaと:orange？
;;;         - dataurl/leaf - 取ると100点？斜めに流れる？
;;;         - :korokke128x96x14 - ？？？
